...
// - Gallery               https://github.com/ocornut/imgui/issues/3075 (please post your
...
/*
 ...
 - 2020/XX/XX (1.XX) - when multi-viewports are enabled, all positions will be in your natural OS
 coordinates space. It means that:
                        - reference to hard-coded positions such as in SetNextWindowPos(ImVec2(0,0))
 are probably not what you want anymore. you may use GetMainViewport()->Pos to offset hard-coded
 positions, e.g. SetNextWindowPos(GetMainViewport()->Pos)
                        - likewise io.MousePos and GetMousePos() will use OS coordinates.
                          If you query mouse positions to interact with non-imgui coordinates you
 will need to offset them, e.g. subtract GetWindowViewport()->Pos.
 - 2020/XX/XX (1.XX) - Moved IME support functions from io.ImeSetInputScreenPosFn,
 io.ImeWindowHandle to the PlatformIO api.
 ...
 directly as they were most unlikely ever used.
 ...
 (github.com/ocornut/imgui/issues/3075). Visuals are ideal as they inspire other programmers. But
...
#      pragma clang diagnostic ignored                                                             \
          "-Wunknown-warning-option" // warning: unknown warning group 'xxx'                      //
                                     // not all warnings are known by all Clang versions and they
                                     // tend to be rename-happy.. so ignoring warnings triggers new
...
#    pragma GCC diagnostic ignored "-Wformat" // warning: format '%p' expects argument of type
                                              // 'void*', but argument 6 has type 'ImGuiWindow*'
#    pragma GCC diagnostic ignored                                                                 \
        "-Wdouble-promotion" // warning: implicit conversion from 'float' to 'double' when passing
...
static const float DOCKING_TRANSPARENT_PAYLOAD_ALPHA =
    0.50f; // For use with io.ConfigDockingTransparentPayload. Apply to Viewport _or_ WindowBg in
           // host viewport.
...
static ImGuiViewportP *AddUpdateViewport(ImGuiWindow *window, ImGuiID id,
                                         const ImVec2 &platform_pos, const ImVec2 &size,
...
ImGuiStyle::ImGuiStyle() {
  ...
  TabMinWidthForUnselectedCloseButton =
      0.0f; // Minimum width for close button to appears on an unselected tab when hovered. Set to
            // 0.0f to always show when hovering, set to FLT_MAX to never show close button unless
            // selected.
  ...
  AntiAliasedLinesUseTex = true; // Enable anti-aliased lines/borders using textures where possible.
...
ImGuiIO::ImGuiIO() {
  ...
  IM_ASSERT(IM_ARRAYSIZE(ImGuiIO::MouseDown) == ImGuiMouseButton_COUNT &&
            IM_ARRAYSIZE(ImGuiIO::MouseClicked) ==
                ImGuiMouseButton_COUNT); // Our pre-C++11 IM_STATIC_ASSERT() macros triggers warning
...
void ImGuiIO::AddInputCharacter(unsigned int c) {
  if (c != 0)
    InputQueueCharacters.push_back(c <= IM_UNICODE_CODEPOINT_MAX ? (ImWchar)c
  ...
  if (InputQueueSurrogate != 0) {
    if ((c & 0xFC00) != 0xDC00) // Invalid low surrogate
      InputQueueCharacters.push_back(IM_UNICODE_CODEPOINT_INVALID);
             ...
             (0xFFFF)) // Codepoint will not fit in ImWchar (extra parenthesis around 0xFFFF somehow
                       // fixes -Wunreachable-code with Clang)
...
ImVec2 ImBezierClosestPoint(const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3, const ImVec2 &p4,
...
static void BezierClosestPointCasteljauStep(const ImVec2 &p, ImVec2 &p_closest, ImVec2 &p_last,
                                            float &p_closest_dist2, float x1, float y1, float x2,
  ...
  if ((d2 + d3) * (d2 + d3) < tess_tol * (dx * dx + dy * dy)) {
    ...
    BezierClosestPointCasteljauStep(p, p_closest, p_last, p_closest_dist2, x1, y1, x12, y12, x123,
                                    y123, x1234, y1234, tess_tol, level + 1);
    BezierClosestPointCasteljauStep(p, p_closest, p_last, p_closest_dist2, x1234, y1234, x234, y234,
...
// automatically.
ImVec2 ImBezierClosestPointCasteljau(const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3,
...
void ImGui::CalcListClipping(int items_count, float items_height, int *out_items_display_start,
  ...
  // We create the union of the ClipRect and the NavScoringRect which at worst should be 1 page away
  // from ClipRect
...
void ImGuiListClipper::Begin(int count, float items_height) {
  ...
  if (ItemsHeight > 0.0f) {
    ...
    if (DisplayStart > 0)
      SetCursorPosYAndSetupForPrevLine(StartPosY + DisplayStart * ItemsHeight,
...
bool ImGuiListClipper::Step() {
  ...
  if (StepNo ==
      2) // Step 2: empty step only required if an explicit items_height was passed to constructor
         // or Begin() and user still call Step(). Does nothing and switch to Step 3.
  {
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags) {
  ...
  // [2017/10/16] Reverted commit 344d48be3 and testing RootWindow instead. I believe it is correct
  // to NOT test for RootWindow but this leaves us unable to use IsItemHovered() after EndChild()
  // itself. Until a solution is found I believe reverting to the test from 2017/09/27 is safe since
  ...
  if ((window->DC.LastItemId == window->ID || window->DC.LastItemId == window->MoveId) &&
...
bool ImGui::ItemHoverable(const ImRect &bb, ImGuiID id) {
  ...
  if (id != 0) {
    ...
    // runtime cost. #define IMGUI_DEBUG_TOOL_ITEM_PICKER_EX in imconfig.h if you want this check to
    // also be performed in ItemAdd().
  ...
  if (g.ActiveId == id && g.FocusTabPressed && !IsActiveIdUsingKey(ImGuiKey_Tab) &&
    ...
    g.FocusRequestNextCounterTabStop =
        ...
        (g.IO.KeyShift ? (is_tab_stop ? -1 : 0)
                       : +1); // Modulo on index will be applied at the end of frame once we've got
...
// We use 'undock_floating_node == false' when dragging from title bar to allow moving groups of
// floating nodes without undocking them.
// - undock_floating_node == true: when dragging from a floating node within a hierarchy, always
// undock the node.
// - undock_floating_node == false: when dragging from a floating node within a hierarchy, move root
// window.
void ImGui::StartMouseMovingWindowOrNode(ImGuiWindow *window, ImGuiDockNode *node,
  ...
  if (node != NULL && node->VisibleWindow &&
    ...
    // Can undock if:
    // - part of a floating node hierarchy with more than one visible node (if only one is visible,
    // we'll just move the whole hierarchy)
    // - part of a dockspace node hierarchy (trivia: undocking from a fixed/central node will create
  ...
  if (g.MovingWindow != NULL) {
    ...
    if (g.IO.MouseDown[0] && IsMousePosValid(&g.IO.MousePos)) {
      ...
      if (moving_window->Pos.x != pos.x || moving_window->Pos.y != pos.y) {
        ...
        if (moving_window
                ->ViewportOwned) // Synchronize viewport immediately because some overlays may
                                 // relies on clipping rectangle before we Begin() into the window.
          moving_window->Viewport->Pos = pos;
      }
      FocusWindow(g.MovingWindow);
    } else {
      // Try to merge the window back into the main viewport.
      // This works because MouseViewport should be != MovingWindow->Viewport on release (as per
      // code in UpdateViewports)
      if (g.ConfigFlagsCurrFrame & ImGuiConfigFlags_ViewportsEnable)
        UpdateTryMergeWindowIntoHostViewport(moving_window, g.MouseViewport);

      // Restore the mouse viewport so that we don't hover the viewport _under_ the moved window
      // during the frame we released the mouse button.
...
void ImGui::UpdateMouseMovingWindowEndFrame() {
  ...
  // (after we're done with all our widgets, so e.g. clicking on docking tab-bar which have set
  // HoveredId already and not get us here!)
...
static void ImGui::UpdateMouseInputs() {
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    ...
    if (g.IO.MouseClicked[i]) // Clicking any mouse button reactivate mouse hovering which may have
...
void ImGui::UpdateTabFocus() {
  ...
  g.FocusTabPressed =
      (g.NavWindow && g.NavWindow->Active && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) &&
       !g.IO.KeyCtrl && IsKeyPressedMap(ImGuiKey_Tab));
  if (g.ActiveId == 0 && g.FocusTabPressed) {
    // Note that SetKeyboardFocusHere() sets the Next fields mid-frame. To be consistent we also
    // manipulate the Next fields even, even though they will be turned into Curr fields by the code
    // below.
...
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
  ...
  // Update io.WantCaptureMouse for the user application (true = dispatch mouse info to imgui, false
  // = dispatch mouse info to Dear ImGui + app)
  ...
  // Update io.WantCaptureKeyboard for the user application (true = dispatch keyboard info to imgui,
  // false = dispatch keyboard info to Dear ImGui + app)
...
#  endif
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.KeysDown); i++)
    g.IO.KeysDownDuration[i] =
        g.IO.KeysDown[i]
            ? (g.IO.KeysDownDuration[i] < 0.0f ? 0.0f : g.IO.KeysDownDuration[i] + g.IO.DeltaTime)
            : -1.0f;
  ...
  // Undocking
  // (needs to be before UpdateMouseMovingWindowNewFrame so the window is already offset and
  // following the mouse on the detaching frame)
  ...
  // custom flags. This fallback is particularly important as it avoid ImGui:: calls from crashing.
...
#  ifdef IMGUI_HAS_DOCK
  ...
  g.PlatformIO.MainViewport = g.Viewports[0]; // Make it accessible in public-facing GetPlatformIO()
                                              // immediately (before the first call to EndFrame)
...
static void AddDrawListToDrawData(ImVector<ImDrawList *> *out_list, ImDrawList *draw_list) {
  ...
  // visible bounds.
  //   Be mindful that the ImDrawList API doesn't filter vertices. Use the Metrics window to inspect
  //   draw list contents.
  ...
  //       Most example back-ends already support this. For example, the OpenGL example code detect
...
static void SetupViewportDrawData(ImGuiViewportP *viewport, ImVector<ImDrawList *> *draw_lists) {
  // When minimized, we report draw_data->DisplaySize as zero to be consistent with non-viewport
  // mode, and to allow applications/back-ends to easily skip rendering.
  // FIXME: Note that we however do NOT attempt to report "zero drawlist / vertices" into the
  // ImDrawData structure. This is because the work has been done already, and its wasted! We should
  // fix that and add optimizations for it earlier in the pipeline, rather than pretend to hide the
  ...
  draw_data->FramebufferScale =
      ImGui::GetIO().DisplayFramebufferScale; // FIXME-VIEWPORT: This may vary on a
...
static void ImGui::EndFrameDrawDimmedBackgrounds() {
  ...
  if (dim_bg_for_window_list) {
    ...
    // (Because the root node renders the background in one shot, in order to avoid flickering when
    // a child dock node is not submitted)
    if (window->RootWindowDockStop->DockIsActive)
      if (window->RootWindow != window->RootWindowDockStop)
        RenderRectFilledWithHole(
            draw_list, window->RootWindow->Rect(), window->RootWindowDockStop->Rect(),
    ...
    if (bb.Contains(window->Viewport->GetMainRect())) // If a window fits the entire viewport,
...
void ImGui::EndFrame() {
  ...
  // Update user-facing viewport list (g.Viewports -> g.PlatformIO.Viewports after filtering out
  // some)
...
void ImGui::Render() {
  ...
  if (g.IO.MouseDrawCursor && g.MouseCursor != ImGuiMouseCursor_None)
    g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &mouse_cursor_offset, &mouse_cursor_size,
                                      &mouse_cursor_uv[0], &mouse_cursor_uv[2]);
  ...
  for (int n = 0; n < g.Viewports.Size; n++) {
    ...
    // (note we scale cursor by current viewport/monitor, however Windows 10 for its own hardware
    ...
    if (mouse_cursor_size.x > 0.0f && mouse_cursor_size.y > 0.0f) {
      ...
      if (viewport->GetMainRect().Overlaps(ImRect(
              g.IO.MousePos,
              g.IO.MousePos + ImVec2(mouse_cursor_size.x + 2, mouse_cursor_size.y + 2) * scale)))
        RenderMouseCursor(GetForegroundDrawList(viewport), g.IO.MousePos, scale, g.MouseCursor,
  ...
  // (Legacy) Call the Render callback function. The current prefer way is to let the user retrieve
  // GetDrawData() and call the render function themselves.
#  ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
static void FindHoveredWindow() {
  ...
  // Special handling for the window being moved: Ignore the mouse viewport check (because it may
  // reset/lose its viewport during the undocking frame)
...
bool ImGui::IsItemToggledSelection() {
  ImGuiContext &g = *GImGui;
  return (g.CurrentWindow->DC.LastItemStatusFlags & ImGuiItemStatusFlags_ToggledSelection) ? true
                                                                                           : false;
}
...
bool ImGui::BeginChildEx(const char *name, ImGuiID id, const ImVec2 &size_arg, bool border,
  ...
  if (g.NavActivateId == id && !(flags & ImGuiWindowFlags_NavFlattened) &&
    ...
    SetActiveID(id + 1, child_window); // Steal ActiveId with another arbitrary id so that key-press
...
void ImGui::EndChild() {
  ...
  if (window->BeginCount > 1) {
    ...
    if (window->AutoFitChildAxises & (1 << ImGuiAxis_X)) // Arbitrary minimum zero-ish child size
...
static ImVec2 CalcWindowAutoFitSize(ImGuiWindow *window, const ImVec2 &size_contents) {
  ...
  if (window->Flags & ImGuiWindowFlags_Tooltip) {
    ...
    if (is_popup || is_menu) // Popups and menus bypass style.WindowMinSize by default, but we give
                             // then a non-zero minimum size to facilitate understanding problematic
...
static bool ImGui::UpdateWindowManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit,
  ...
  // Clip mouse interaction rectangles within the viewport rectangle (in practice the narrowing is
  // going to happen most of the time).
  // - Not narrowing would mostly benefit the situation where OS windows _without_ decoration have a
  // threshold for hovering when outside their limits.
  //   This is however not the case with current back-ends under Win32, but a custom borderless
  //   window implementation would benefit from it.
  // - When decoration are enabled we typically benefit from that distance, but then our resize
  // elements would be conflicting with OS resize elements, so we also narrow.
  // - Note that we are unable to tell if the platform setup allows hovering with a distance
  // threshold (on Win32, decorated window have such threshold). We only clip interaction so we
  // overwrite window->ClipRect, cannot call PushClipRect() yet as DrawList is not yet setup.
  ...
  for (int border_n = 0; border_n < resize_border_count; border_n++) {
    ...
    // GetForegroundDrawLists(window)->AddRect(border_rect.Min, border_rect.Max, IM_COL32(255, 255,
    // 0, 255));
...
void ImGui::RenderWindowDecorations(ImGuiWindow *window, const ImRect &title_bar_rect,
  ...
  if (window->Collapsed) {
    ...
    // Title bar
    // (when docked, DockNode are drawing their own title bar. Individual windows however do NOT set
    // the _NoTitleBar flag, in order for their pos/size to be matching their undocking state.)
    ...
    if (flags & ImGuiWindowFlags_MenuBar) {
      ...
      window->DrawList->AddRectFilled(
          ...
          (flags & ImGuiWindowFlags_NoTitleBar) ? window_rounding : 0.0f, ImDrawCornerFlags_Top);
    ...
    // Docking: Unhide tab bar (small triangle in the corner), drag from small triangle to quickly
    // undock
    ...
    if (window->DockIsActive && node->IsHiddenTabBar() && !node->IsNoTabBar()) {
      ...
      // FIXME-DOCK: Ideally we'd use ImGuiCol_TitleBgActive/ImGuiCol_TitleBg here, but neither is
      // guaranteed to be visible enough at this sort of size..
      ...
      window->DrawList->AddTriangleFilled(p, p + ImVec2(unhide_sz_draw, 0.0f),
                                          p + ImVec2(0.0f, unhide_sz_draw), col);
    }
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow *window, const ImRect &title_bar_rect,
  ...
  // Close/Collapse button, while uncentered title text will still reach edges correct.
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
  ...
  // Docking
  // (NB: during the frame dock nodes are created, it is possible that (window->DockIsActive ==
  // false) even though (window->DockNode->Windows.Size > 1)
  ...
  if (first_begin_of_the_frame) {
    ...
    else if (g.NavWindowingListWindow != NULL && (window->Flags & ImGuiWindowFlags_NoNavFocus) ==
    ...
    // SELECT VIEWPORT
    // We need to do this before using any style/font sizes, as viewport with a different DPI may
    // affect font sizes.
    ...
    // inconsistencies) We read Style data after the call to UpdateSelectWindowViewport() which
    // might be swapping the style.
    ...
    if (window->ViewportAllowPlatformMonitorExtend >= 0 && !window->ViewportOwned &&
      ...
      if (!window->Viewport->GetMainRect().Contains(window->Rect())) {
        // This is based on the assumption that the DPI will be known ahead (same as the DPI of the
        // selection done in UpdateSelectWindowViewport)
        // ImGuiViewport* old_viewport = window->Viewport;
        window->Viewport = AddUpdateViewport(window, window->ID, window->Pos, window->Size,
                                             ImGuiViewportFlags_NoFocusOnAppearing);

        // FIXME-DPI
        // IM_ASSERT(old_viewport->DpiScale == window->Viewport->DpiScale); // FIXME-DPI: Something
        // went wrong
        SetCurrentViewport(window, window->Viewport);
        window->FontDpiScale = (g.IO.ConfigFlags & ImGuiConfigFlags_DpiEnableScaleFonts)
                                   ? window->Viewport->DpiScale
    ...
    if (window->ViewportOwned) {
      ...
      // Synchronize viewport -> window in case the platform window has been moved or resized from
      // the OS/WM
      ...
      // Either a SetNextWindowPos() call in the current frame or a SetWindowPos() call in the
      // previous frame may have this effect.
      ...
      const bool         is_short_lived_floating_window =
          (flags &
           (ImGuiWindowFlags_ChildMenu | ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_Popup)) != 0;
      ...
      // _NoFocusOnClick so that clicking on them won't steal the OS focus away from their parent
      // window (which may be reflected in OS the title bar decoration). Setting _NoFocusOnClick
      // would technically prevent us from bringing back to front in case they are being covered by
      // an OS window from a different app, but it shouldn't be much of a problem considering those
      ...
      else if ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) &&
               parent_window_in_stack)
      ...
      // We also tell the back-end that clearing the platform window won't be necessary, as our
      // window is filling the viewport and we have disabled BgAlpha
      if (!(flags & ImGuiWindowFlags_NoBackground))
    ...
    if (!window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) &&
      ...
      if (!window->ViewportOwned && viewport_rect.GetWidth() > 0 &&
        ...
        if (window->Viewport->PlatformMonitor == -1) {
          // Fallback for "lost" window (e.g. a monitor disconnected): we move the window back over
          // the main viewport
    ...
    // Synchronize window --> viewport again and one last time (clamping and manual resize may have
    // affected either)
    if (window->ViewportOwned) {
    ...
    // Save last known viewport position within the window itself (so it can be saved in .ini file
    // and restored)
    ...
    // Draw modal or window list full viewport dimming background (for other viewports we'll render
    // them in EndFrame)
    ...
    // the parent window has zero vertices, which is a common pattern leading to laying out multiple
    // overlapping child. We also disabled this when we have dimming overlay behind this specific
    // one child.
    ...
    if (p_open != NULL && window->Viewport->PlatformRequestClose &&
      ...
      if (!window->DockIsActive || window->DockTabIsVisible) {
        ...
        g.NavWindowingToggleLayer =
            false; // Assume user mapped PlatformRequestClose on ALT-F4 so we disable ALT for menu
                   // toggle. False positive not an issue.
    ...
    */
    ...
    if (g.IO.ConfigFlags & ImGuiConfigFlags_DockingEnable) {
      // Docking: Dragging a dockable window (or any of its child) turns it into a drag and drop
      // source. We need to do this _before_ we overwrite window->DC.LastItemId below because
...
#  endif
  ...
  // When we are about to select this tab (which will only be visible on the _next frame_), flag it
  // with a non-zero HiddenFramesCannotSkipItems. This will have the important effect of actually
  // returning true in Begin() and not setting SkipItems, allowing an earlier submission of the
  // window contents. This is analogous to regular windows being hidden from one frame. It is
  // especially important as e.g. nested TabBars would otherwise generate flicker in the form of one
  // empty frame, or focus requests won't be processed.
  if (window->DockIsActive && !window->DockTabIsVisible) {
...
void ImGui::End() {
  ...
  if (window->DockNode && window->DockTabIsVisible)
    if (ImGuiWindow *host_window = window->DockNode->HostWindow) // FIXME-DOCK
      host_window->DC.CursorMaxPos =
          window->DC.CursorMaxPos + window->WindowPadding - host_window->WindowPadding;
...
void ImGui::FocusWindow(ImGuiWindow *window) {
  ...
  // - Using dock host items (tab, collapse button) can trigger this before we redirect the
  // ActiveIdWindow toward the child window.
  if (g.ActiveId != 0 && g.ActiveIdWindow &&
      g.ActiveIdWindow->RootWindowDockStop != focus_front_window)
  ...
  for (int i = start_idx; i >= 0; i--) {
    ...
    if (window != ignore_window && window->WasActive && window->RootWindowDockStop == window)
      if ((window->Flags & (ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_NoNavInputs)) !=
        ...
        // window->NavLastChildNavWindow will still point to B. We might leverage the tab order
        // implicitly stored in window->DockNodeAsHost->TabBar (essentially the
        // 'most_recently_selected_tab' code in tab bar will do that but on next update) to tell
        // which is the "previous" window. Or we may leverage
        // 'LastFrameFocused/LastFrameJustFocused' and have this function handle child window
        // itself?
...
bool ImGui::IsWindowNavFocusable(ImGuiWindow *window) {
  return window->Active && window == window->RootWindowDockStop &&
...
static void ImGui::ErrorCheckNewFrameSanityChecks() {
  ...
  IM_ASSERT(g.Initialized);
  g.IO.DeltaTime = g.IO.DeltaTime > 1.0e-3f ? g.IO.DeltaTime : 1.0e-3f;
  ...
  // Perform simple check: error if Docking or Viewport are enabled _exactly_ on frame 1 (instead of
  // frame 0 or later), which is a common error leading to loss of .ini data.
  if (g.FrameCount == 1 && (g.IO.ConfigFlags & ImGuiConfigFlags_DockingEnable) &&
      (g.ConfigFlagsLastFrame & ImGuiConfigFlags_DockingEnable) == 0)
    IM_ASSERT(0 && "Please set DockingEnable before the first call to NewFrame()! Otherwise you "
                   "will lose your .ini settings!");
  if (g.FrameCount == 1 && (g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) &&
    ...
    IM_ASSERT(0 && "Please set ViewportsEnable before the first call to NewFrame()! Otherwise you "
                   "will lose your .ini settings!");
...
#  ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
      IM_ASSERT(g.IO.RenderDrawListsFn ==
                NULL); // Call ImGui::Render() then pass ImGui::GetDrawData() yourself to your
                       // render function!
#  endif
    } else {
    ...
    // Perform simple checks on platform monitor data + compute a total bounding box for quick early
    // outs
    for (int monitor_n = 0; monitor_n < g.PlatformIO.Monitors.Size; monitor_n++) {
      ...
      IM_ASSERT(ImRect(mon.MainPos, mon.MainPos + mon.MainSize)
                ...
                "Monitor work bounds not setup properly. If you don't have work area information, "
                "just copy MainPos/MainSize into them.");
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow *window, bool write) {
  ...
  // Window stacks
  // NOT checking: DC.ItemWidth, DC.AllowKeyboardFocus, DC.ButtonRepeat, DC.TextWrapPos (per window)
  // to allow user to conveniently push once and not pop (they are cleared on Begin)
  {
...
void ImGui::EndGroup() {
  ...
  // be neater if we replaced window.DC.LastItemId by e.g. 'bool LastItemIsActive', but would put a
  // little more burden on individual widgets. Also if you grep for LastItemId you'll notice it is
  // only used in that context. (The tests not symmetrical because ActiveIdIsAlive is an ID itself,
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow *window) {
  ...
  if (window->ScrollTarget.y < FLT_MAX) {
    ...
    scroll.y =
        target_y - cr_y * (window->SizeFull.y - window->ScrollbarSizes.y - decoration_up_height);
  }
...
void ImGui::SetScrollFromPosY(ImGuiWindow *window, float local_y, float center_y_ratio) {
  ...
  local_y -= window->TitleBarHeight() +
             window->MenuBarHeight(); // FIXME: Would be nice to have a more standardized access to
                                      // our scrollable/client rect
...
static float CalcScrollSnap(float target, float snap_min, float snap_max, float snap_threshold,
                            float center_ratio) {
...
void ImGui::SetScrollHereX(float center_x_ratio) {
  ...
  float         target_x  = ImLerp(window->DC.LastItemRect.Min.x - spacing_x,
  ...
  const float snap_x_max =
      window->DC.CursorStartPos.x + window->ContentSize.x + window->WindowPadding.x;
...
void ImGui::SetScrollHereY(float center_y_ratio) {
  ...
  const float snap_y_max =
      window->DC.CursorStartPos.y + window->ContentSize.y + window->WindowPadding.y;
...
bool ImGui::BeginPopupModal(const char *name, bool *p_open, ImGuiWindowFlags flags) {
  ...
  if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos) == 0) {
    ...
    SetNextWindowPos(viewport->GetMainRect().GetCenter(), ImGuiCond_FirstUseEver,
                     ImVec2(0.5f, 0.5f));
  }
...
// - This is essentially the same as calling OpenPopupContextItem() + BeginPopup() but written to
// avoid
...
ImRect ImGui::GetWindowAllowedExtentRect(ImGuiWindow *window) {
  ...
  if (window->ViewportAllowPlatformMonitorExtend >= 0) {
    // Extent with be in the frame of reference of the given viewport (so Min is likely to be
    // negative here)
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow *window) {
  ...
  if (window->Flags & ImGuiWindowFlags_Tooltip) {
    ...
    ImVec2 pos = FindBestWindowPosForPopupEx(ref_pos, window->Size, &window->AutoPosLastDirection,
                                             r_outer, r_avoid);
    if (window->AutoPosLastDirection == ImGuiDir_None)
      pos = ref_pos +
            ImVec2(2, 2); // If there's not enough room, for tooltip we prefer avoiding the cursor
                          // at all cost even if it means that part of the tooltip won't be visible.
    return pos;
  }
  IM_ASSERT(0);
  return window->Pos;
...
void ImGui::SetNavIDWithRectRel(ImGuiID id, int nav_layer, ImGuiID focus_scope_id,
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult *result, ImRect cand) {
  ...
  // We perform scoring on items bounding box clipped by the current clipping rectangle on the other
  // axis (clipping on our movement axis would give us equal scores for all clipped items) For
  // example, this ensure that items in one column are not reached when moving vertically from items
  // in another column.
...
static void ImGui::NavProcessItem(ImGuiWindow *window, const ImRect &nav_bb, const ImGuiID id) {
  ImGuiContext &g = *GImGui;
  // if (!g.IO.NavActive)  // [2017/10/06] Removed this possibly redundant test but I am not sure of
  // all the side-effects yet. Some of the feature here will need to work regardless of using a
  // _NoNavInputs flag.
  //    return;
  ...
  // FIXME-NAV: Consider policy for double scoring (scoring from NavScoringRectScreen + scoring from
  // a rect wrapped according to current wrapping policy)
...
#  endif
  ...
  if (g.NavId == id) {
    g.NavWindow = window; // Always refresh g.NavWindow, because some operations such as FocusItem()
                          // don't have a window.
...
static void ImGui::NavSaveLastChildNavWindowIntoParent(ImGuiWindow *nav_window) {
  ...
  while (parent && parent->RootWindowDockStop != parent &&
         (parent->Flags & ImGuiWindowFlags_ChildWindow) != 0 &&
...
static ImGuiWindow *ImGui::NavRestoreLastChildNavWindow(ImGuiWindow *window) {
  ...
  if (window->DockNodeAsHost && window->DockNodeAsHost->TabBar)
    if (ImGuiTabItem *tab =
            TabBarFindMostRecentlySelectedTabForActiveWindow(window->DockNodeAsHost->TabBar))
      return tab->Window;
  return window;
}
...
void ImGui::NavInitWindow(ImGuiWindow *window, bool force_reinit) {
  ...
  if (!(window->Flags & ImGuiWindowFlags_NoNavInputs))
    if (!(window->Flags & ImGuiWindowFlags_ChildWindow) ||
        (window->Flags & ImGuiWindowFlags_Popup) || (window->NavLastIds[0] == 0) || force_reinit)
      init_for_nav = true;
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode) {
  ...
  if (t < 0.0f && mode == ImGuiInputReadMode_Released) // Return 1.0f when just released, no repeat,
                                                       // ignore analog input.
...
#  endif
  ...
  if (nav_gamepad_active)
    if (g.IO.NavInputs[ImGuiNavInput_Activate] > 0.0f ||
        g.IO.NavInputs[ImGuiNavInput_Input] > 0.0f || g.IO.NavInputs[ImGuiNavInput_Cancel] > 0.0f ||
...
#  define NAV_MAP_KEY(_KEY, _NAV_INPUT)                                                            \
    ...
    if (g.IO.KeyAlt && !g.IO.KeyCtrl) // AltGR is Alt+Ctrl, also even on keyboards without AltGR we
...
#  undef NAV_MAP_KEY
  ...
  // Store our return window (for returning from Layer 1 to Layer 0) and clear it as soon as we step
  // back in our own Layer 0
  ...
  g.IO.NavVisible =
      (g.IO.NavActive && g.NavId != 0 && !g.NavDisableHighlight) || (g.NavWindowingTarget != NULL);
  ...
  if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) &&
    ...
    const float  scroll_speed =
        IM_ROUND(window->CalcFontSize() * 100 *
                 g.IO.DeltaTime); // We need round the scrolling speed because sub-pixel scroll
  ...
  ImRect nav_rect_rel = (g.NavWindow && !g.NavWindow->NavRectRel[g.NavLayer].IsInverted())
                            ...
                            : ImRect(0, 0, 0, 0);
  g.NavScoringRect =
      g.NavWindow ? ImRect(g.NavWindow->Pos + nav_rect_rel.Min, g.NavWindow->Pos + nav_rect_rel.Max)
                  : ImRect(0, 0, 0, 0);
...
static float ImGui::NavUpdatePageUpPageDown() {
  ...
  const bool   page_up_held =
      IsKeyDown(g.IO.KeyMap[ImGuiKey_PageUp]) && !IsActiveIdUsingKey(ImGuiKey_PageUp);
  const bool page_down_held =
      IsKeyDown(g.IO.KeyMap[ImGuiKey_PageDown]) && !IsActiveIdUsingKey(ImGuiKey_PageDown);
  const bool home_pressed =
      IsKeyPressed(g.IO.KeyMap[ImGuiKey_Home]) && !IsActiveIdUsingKey(ImGuiKey_Home);
  const bool end_pressed =
      IsKeyPressed(g.IO.KeyMap[ImGuiKey_End]) && !IsActiveIdUsingKey(ImGuiKey_End);
      ...
      home_pressed != end_pressed) // If either (not both) are pressed
  {
    if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
      ...
      if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true)) {
        ...
        // ImGuiNavMoveFlags_ScrollToEdge flag, we don't scroll immediately to avoid scrolling
...
static void ImGui::NavUpdateWindowing() {
  ...
  if (apply_focus_window &&
      (g.NavWindow == NULL || apply_focus_window != g.NavWindow->RootWindowDockStop)) {
  ...
  if (apply_toggle_layer && g.NavWindow) {
    ...
    // It however persist on docking tab tabs.
...
void ImGui::NavUpdateWindowingOverlay() {
  ...
  ImGuiViewportP *viewport =
      /*g.NavWindow ? g.NavWindow->Viewport :*/ (ImGuiViewportP *)GetMainViewport();
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags) {
  ...
  if (!(flags & ImGuiDragDropFlags_SourceExtern)) {
    ...
    if (source_id == 0) {
      ...
      // the ImGuiDragDropFlags_SourceAllowNullID flag, C) Swallow your programmer pride.
...
void ImGui::LogRenderedText(const ImVec2 *ref_pos, const char *text, const char *text_end) {
  ...
  for (;;) {
    // Split the string. Each new line (after a '\n') is followed by spacing corresponding to the
    // current depth of our log entry. We don't add a trailing \n to allow a subsequent item on the
...
static void WindowSettingsHandler_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler,
  ...
  for (ImGuiWindowSettings *settings = g.SettingsWindows.begin(); settings != NULL;
    ...
    if (settings->DockId != 0) {
      // Write DockId as 4 digits if possible. Automatic DockId are small numbers, but full explicit
      // DockSpace() are full ImGuiID range.
...
void ImGui::SetCurrentViewport(ImGuiWindow *current_window, ImGuiViewportP *viewport) {
  ...
  // current_window->Name : NULL, viewport ? viewport->ID : 0);
...
static bool ImGui::GetWindowAlwaysWantOwnViewport(ImGuiWindow *window) {
  // Tooltips and menus are not automatically forced into their own viewport when the NoMerge flag
  // is set, however the multiplication of viewports makes them more likely to protrude and create
  // their own.
  ImGuiContext &g = *GImGui;
  if (g.IO.ConfigViewportsNoAutoMerge ||
    ...
    if (g.ConfigFlagsCurrFrame & ImGuiConfigFlags_ViewportsEnable)
      if (!window->DockIsActive)
        if ((window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_ChildMenu |
...
// (This additionally keeps windows at the same place when ImGuiConfigFlags_ViewportsEnable is
// toggled!)
void ImGui::TranslateWindowsInViewport(ImGuiViewportP *viewport, const ImVec2 &old_pos,
                                       const ImVec2 &new_pos) {
  ImGuiContext &g = *GImGui;
  IM_ASSERT(viewport->Window == NULL && (viewport->Flags & ImGuiViewportFlags_CanHostOtherWindows));

  // 1) We test if ImGuiConfigFlags_ViewportsEnable was just toggled, which allows us to
  // conveniently translate imgui windows from OS-window-local to absolute coordinates or
  // vice-versa. 2) If it's not going to fit into the new size, keep it at same absolute position.
  // One problem with this is that most Win32 applications doesn't update their render while
  // dragging, and so the window will appear to teleport when releasing the mouse.
  ...
  for (int window_n = 0; window_n < g.Windows.Size; window_n++) // FIXME-OPT
    if (translate_all_windows || (g.Windows[window_n]->Viewport == viewport &&
...
// If the back-end doesn't set MouseLastHoveredViewport or doesn't honor
// ImGuiViewportFlags_NoInputs, we do a search ourselves. A) It won't take account of the
// possibility that non-imgui windows may be in-between our dragged window and our target window. B)
...
// Note that this is running even if 'ImGuiConfigFlags_ViewportsEnable' is not set, in order to
// clear unused viewports (if any) and update monitor info.
static void ImGui::UpdateViewportsNewFrame() {
  ImGuiContext &g = *GImGui;
  IM_ASSERT(g.PlatformIO.Viewports.Size <= g.Viewports.Size);

  // Update Minimized status (we need it first in order to decide if we'll apply Pos/Size of the
  // main viewport)
  ...
  // FIXME-VIEWPORT: Size is driven by back-end/user code for backward-compatibility but we should
  ...
  if (viewports_enabled && (main_viewport->Flags & ImGuiViewportFlags_Minimized)) {
    main_viewport_pos =
        main_viewport->Pos; // Preserve last pos/size when minimized (FIXME: We don't do the same
  ...
  for (int n = 0; n < g.Viewports.Size; n++) {
    ...
    if (viewports_enabled) {
      ...
      // We do it early in the frame instead of waiting for UpdatePlatformWindows() to avoid a frame
      // of lag when moving/resizing using OS facilities.
    ...
    if (viewport->DpiScale != 0.0f && new_dpi_scale != viewport->DpiScale) {
      ...
      // Scale our window moving pivot so that the window will rescale roughly around the mouse
      // position.
      // FIXME-VIEWPORT: This currently creates a resizing feedback loop when a window is straddling
      // a DPI transition border. (Minor: since our sizes do not perfectly linearly scale, deferring
      // the click offset scale until we know the actual window scale ratio may get us slightly more
  ...
  // Mouse handling: decide on the actual mouse viewport for this frame between the active/focused
  // viewport and the hovered viewport. Note that 'viewport_hovered' should skip over any viewport
  // that has the ImGuiViewportFlags_NoInputs flags set.
  ...
  if (g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
    ...
    // If the back-end doesn't know how to honor ImGuiViewportFlags_NoInputs, we do a search
    // ourselves. Note that this search: A) won't take account of the possibility that non-imgui
    // windows may be in-between our dragged window and our target window. B) uses
    // LastFrameAsRefViewport as a flawed replacement for the last time a window was focused (we
  ...
  // (when moving a window we aim at its viewport, but this will be overwritten below if we go in
  // drag and drop mode)
  if (g.MovingWindow)
  ...
  // - when we are between viewports, our dragged preview will tend to show in the last viewport
  // _even_ if we don't have tooltips in their viewports (when lacking monitor info)
  // - consider the case of holding on a menu item to browse child menus: even thou a mouse button
  // is held, there's no active id because menu items only react on mouse release.
...
static void ImGui::UpdateViewportsEndFrame() {
  ...
  g.Viewports[0]->ClearRequestFlags(); // Clear main viewport flags because UpdatePlatformWindows()
                                       // won't do it and may not even be called
}

// FIXME: We should ideally refactor the system to call this every frame (we currently don't)
ImGuiViewportP *ImGui::AddUpdateViewport(ImGuiWindow *window, ImGuiID id, const ImVec2 &pos,
  ...
  if (viewport) {
    ...
    viewport->Flags =
        flags | (viewport->Flags & ImGuiViewportFlags_Minimized); // Preserve existing flags
  } else {
    // New viewport
    ...
    // We normally setup for all viewports in NewFrame() but here need to handle the mid-frame
    // creation of a new viewport. We need to extend the fullscreen clip rect so the OverlayDrawList
    // clip is correct for that the first frame
    ...
    // Store initial DpiScale before the OS platform window creation, based on expected monitor
    // data. This is so we can select an appropriate font size on the first frame of our window
    // lifetime
...
static void ImGui::UpdateSelectWindowViewport(ImGuiWindow *window) {
  ...
  if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasViewport) == 0) {
    ...
    // Attempt to restore saved viewport id (= window that hasn't been activated yet), try to
    // restore the viewport based on saved 'window->ViewportPos' restored from .ini file
...
void ImGui::UpdatePlatformWindows() {
  ...
  for (int i = 1; i < g.Viewports.Size; i++) {
    ...
    // Destroy platform window if the viewport hasn't been submitted or if it is hosting a hidden
    // window (the implicit/fallback Debug##Default window will be registering its viewport then be
    // disabled, causing a dummy DestroyPlatformWindow to be made each frame)
    ...
    // New windows that appears directly in a new viewport won't always have a size on their first
    // frame
    ...
    if (is_new_platform_window) {
      ...
      viewport->LastRendererSize =
          viewport->Size; // We don't need to call Renderer_SetWindowSize() as it is expected
    ...
    // Optional, general purpose call to allow the back-end to perform general book-keeping even if
    // things haven't changed.
    if (g.PlatformIO.Platform_UpdateWindow) g.PlatformIO.Platform_UpdateWindow(viewport);

    if (is_new_platform_window) {
      // On startup ensure new platform window don't steal focus (give it a few frames, as nested
      // contents may lead to viewport being created a few frames late)
      ...
      // This is useful for our platform z-order heuristic when io.MouseHoveredViewport is not
      // available.
  ...
  // Update our implicit z-order knowledge of platform windows, which is used when the back-end
  // cannot provide io.MouseHoveredViewport. When setting Platform_GetWindowFocus, it is expected
  // that the platform back-end can handle calls without crashing if it doesn't have data stored.
...
// Custom renderers may prefer to not call this function at all, and instead iterate the publicly
// exposed platform data and handle rendering/sync themselves. The Render/Swap functions stored in
// ImGuiPlatformIO are merely here to allow for this helper to exist, but you can do it yourself:
//
...
static int ImGui::FindPlatformMonitorForRect(const ImRect &rect) {
  ...
  // Use a minimum threshold of 1.0f so a zero-sized rect won't false positive, and will still find
  // the correct monitor given its position. This is necessary for tooltips which always resize down
  // to zero at first.
  ...
  for (int monitor_n = 0;
       monitor_n < g.PlatformIO.Monitors.Size && best_monitor_surface < surface_threshold;
       monitor_n++) {
    const ImGuiPlatformMonitor &monitor = g.PlatformIO.Monitors[monitor_n];
...
// Update monitor from viewport rectangle (we'll use this info to clamp windows and save windows
// lost in a removed monitor)
...
void ImGui::DestroyPlatformWindow(ImGuiViewportP *viewport) {
  ...
  if (viewport->PlatformWindowCreated) {
    ...
    // Don't clear PlatformWindowCreated for the main viewport, as we initially set that up to true
    // in Initialize() The right-er way may be to leave it to the back-end to set this flag
    // all-together, and made the flag public.
...
void ImGui::DestroyPlatformWindows() {
  ...
  // chance to the back-end to clear any data they may have stored in e.g. PlatformUserData,
  // RendererUserData. It is convenient for the platform back-end code to store something in the
  // main viewport, in order for e.g. the mouse handling code to operator a consistent manner. It is
  // expected that the back-end can handle calls to Renderer_DestroyWindow/Platform_DestroyWindow
...
struct ImGuiDockRequest {
  ...
  ImGuiWindow
      *DockTargetWindow; // Destination/Target Window to dock into (may be a loose window or a
                         // DockNode, might be NULL in which case DockTargetNode cannot be NULL)
...
struct ImGuiDockPreviewData {
  ...
  bool IsSplitDirExplicit; // Set when hovered the drop rect (vs. implicit SplitDir==None when
                           // hovered the window)
  ImGuiDockNode *SplitNode;
  ImGuiDir       SplitDir;
  float          SplitRatio;
  ImRect DropRectsDraw[ImGuiDir_COUNT + 1]; // May be slightly different from hit-testing drop rects
                                            // used in DockNodeCalcDropRects()
...
static void    DockNodePreviewDockSetup(ImGuiWindow *host_window, ImGuiDockNode *host_node,
...
static void    DockNodeCalcTabBarLayout(const ImGuiDockNode *node, ImRect *out_title_rect,
                                        ImRect *out_tab_bar_rect, ImVec2 *out_window_menu_button_pos);
static void    DockNodeCalcSplitRects(ImVec2 &pos_old, ImVec2 &size_old, ImVec2 &pos_new,
                                      ImVec2 &size_new, ImGuiDir dir, ImVec2 size_new_desired);
static bool    DockNodeCalcDropRectsAndTestMousePos(const ImRect &parent, ImGuiDir dir,
                                                    ImRect &out_draw, bool outer_docking,
...
static void  DockSettingsHandler_ReadLine(ImGuiContext *, ImGuiSettingsHandler *, void *entry,
                                          const char *line);
...
// Docking: ImGuiDockContext
//-----------------------------------------------------------------------------
// The lifetime model is different from the one of regular windows: we always create a ImGuiDockNode
// for each ImGuiDockNodeSettings, or we always hold the entire docking node tree. Nodes are
// frequently hidden, e.g. if the window(s) or child nodes they host are not active. At boot time
// only, we run a simple GC to remove nodes that have no references. Because dock node settings
// (which are small, contiguous structures) are always mirrored by their corresponding dock nodes
// (more complete structures), we can also very easily recreate the nodes from scratch given the
// settings data (this is what DockContextRebuild() does). This is convenient as docking
// reconfiguration can be implemented by mostly poking at the simpler settings data.
//-----------------------------------------------------------------------------
...
// [DEBUG] This function also acts as a defacto test to make sure we can rebuild from scratch
// without a glitch (Different from DockSettingsHandler_ClearAll() + DockSettingsHandler_ApplyAll()
// because this reuses current settings!)
...
void ImGui::DockContextUpdateDocking(ImGuiContext *ctx) {
  ...
  // Store hovered dock node. We could in theory use DockNodeTreeFindVisibleNodeByPos() on the root
  // host dock node, but using ->DockNode is a good shortcut.
  ...
  if (ImGuiWindow *hovered_window = g.HoveredWindowUnderMovingWindow) {
    if (hovered_window->DockNode)
      ...
      g.HoveredDockNode =
          DockNodeTreeFindVisibleNodeByPos(hovered_window->DockNodeAsHost, g.IO.MousePos);
  }
  ...
  // We can have NULL pointers when we delete nodes, but because ID are recycled this should
  // amortize nicely (and our node count will never be very high)
  for (int n = 0; n < dc->Nodes.Data.Size; n++)
    if (ImGuiDockNode *node = (ImGuiDockNode *)dc->Nodes.Data[n].val_p)
...
ImGuiID ImGui::DockContextGenNodeID(ImGuiContext *ctx) {
  // Generate an ID for new node (the exact ID value doesn't matter as long as it is not already
  // used)
  // FIXME-OPT FIXME-DOCK: This is suboptimal, even if the node count is small enough not to be a
  // worry. We should poke in ctx->Nodes to find a suitable ID faster.
  ImGuiID id = 0x0001;
...
static ImGuiDockNode *ImGui::DockContextAddNode(ImGuiContext *ctx, ImGuiID id) {
  // Generate an ID for the new node (the exact ID value doesn't matter as long as it is not already
  // used) and add the first window.
  if (id == 0)
    id = DockContextGenNodeID(ctx);
  else
    IM_ASSERT(DockContextFindNodeByID(ctx, id) == NULL);

  // We don't set node->LastFrameAlive on construction. Nodes are always created at all time to
  // reflect .ini settings!
...
// Pre C++0x doesn't allow us to use a function-local type (without linkage) as template parameter,
  ...
  for (int settings_n = 0; settings_n < dc->NodesSettings.Size; settings_n++) {
    ...
    remove |=
        (data->CountWindows == 1 && settings->ParentNodeId == 0 && data->CountChildNodes == 0 &&
         !(settings->Flags &
           ImGuiDockNodeFlags_CentralNode)); // Floating root node with only 1 window
...
static void ImGui::DockContextBuildNodesFromSettings(ImGuiContext *         ctx,
  ...
  for (int node_n = 0; node_n < node_settings_count; node_n++) {
    ...
    node->ParentNode =
        settings->ParentNodeId ? DockContextFindNodeByID(ctx, settings->ParentNodeId) : NULL;
    ...
    // This is useful as the RootWindowForTitleBarHighlight links necessary to highlight the
    // currently focused node requires node->HostWindow to be set.
...
void ImGui::DockContextBuildAddWindowsToNodes(ImGuiContext *ctx, ImGuiID root_id) {
  // Rebind all windows to nodes (they can also lazily rebind but we'll have a visible glitch during
  // the first frame)
  ImGuiContext &g = *ctx;
  for (int n = 0; n < g.Windows.Size; n++) {
    ImGuiWindow *window = g.Windows[n];
    ...
    IM_ASSERT(node !=
              NULL); // This should have been called after DockContextBuildNodesFromSettings()
...
void ImGui::DockContextProcessDock(ImGuiContext *ctx, ImGuiDockRequest *req) {
  ...
  if (payload_window) {
    ...
    payload_window->DockNodeAsHost = NULL; // Important to clear this as the node will have its life
                                           // as a child which might be merged/deleted later.
  ...
  // FIXME-DOCK: When we are trying to dock an existing single-window node into a loose window,
  // transfer Node ID as well When processing an interactive split, usually LastFrameAlive will be <
  // g.FrameCount. But DockBuilder operations can make it ==.
  ...
  if (node != payload_node) {
    // Create tab bar before we call DockNodeMoveWindows (which would attempt to move the old
    // tab-bar, which would lead us to payload tabs wrongly appearing before target tabs!)
    ...
    if (payload_node != NULL) {
      ...
      if (payload_node->IsSplitNode()) {
        if (node->Windows.Size > 0) {
          // We can dock a split payload into a node that already has windows _only_ if our payload
          // is a node tree with a single visible node. In this situation, we move the windows of
          // the target node into the currently visible node of the payload. This allows us to
          // preserve some of the underlying dock tree settings nicely.
          IM_ASSERT(payload_node->OnlyNodeWithWindows !=
                    NULL); // The docking should have been blocked by DockNodePreviewDockSetup()
...
void ImGui::DockContextProcessUndockNode(ImGuiContext *ctx, ImGuiDockNode *node) {
  ...
  if (node->IsRootNode() || node->IsCentralNode()) {
    // In the case of a root node or central node, the node will have to stay in place. Create a new
    // node to receive the payload.
    ...
    node->ParentNode->AuthorityForViewport =
        ImGuiDataAuthority_Window; // The node that stays in place keeps the viewport, so our newly
                                   // dragged out node will create a new viewport
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode *node, ImGuiWindow *window,
  ...
  // If more than 2 windows appeared on the same frame, we'll create a new hosting DockNode from the
  // point of the second window submission. Then we need to hide the first window (after its been
  // output) otherwise it would be visible as a standalone window for one frame.
  ...
  // When reactivating a node with one or two loose window, the window pos/size/viewport are
  // authoritative over the node storage. In particular it is important we init the viewport from
  // the first window so we don't create two viewports and drop one.
  ...
  // Update this without waiting for the next time we Begin() in the window, so our host window will
  // have the proper title bar color on its first frame.
  if (node->HostWindow)
    UpdateWindowParentAndRootLinks(window, window->Flags | ImGuiWindowFlags_ChildWindow,
                                   node->HostWindow);
}

static void ImGui::DockNodeRemoveWindow(ImGuiDockNode *node, ImGuiWindow *window,
...
static void ImGui::DockNodeMoveWindows(ImGuiDockNode *dst_node, ImGuiDockNode *src_node) {
  ...
  // If the dst_node is empty we can just move the entire tab bar (to preserve selection, scrolling,
  // etc.)
...
static void ImGui::DockNodeApplyPosSizeToWindows(ImGuiDockNode *node) {
  for (int n = 0; n < node->Windows.Size; n++) {
    SetWindowPos(node->Windows[n], node->Pos,
                 ImGuiCond_Always); // We don't assign directly to Pos because it can break the
...
static void ImGui::DockNodeUpdateVisibleFlagAndInactiveChilds(ImGuiDockNode *node) {
  ...
  // There is the possibility that one of our child becoming empty will delete itself and moving its
  // sibling contents into 'node'. If 'node->ChildNode[0]' delete itself, then
  // 'node->ChildNode[1]->Windows' will be moved into 'node' If 'node->ChildNode[1]' delete itself,
  // then 'node->ChildNode[0]->Windows' will be moved into 'node' and the "remove inactive windows"
  // loop will have run twice on those windows (harmless)
  ...
  for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
    ...
    remove |= node_was_active && (node->WantCloseAll || node->WantCloseTabId == window->ID) &&
              ...
              !(window->Flags &
                ImGuiWindowFlags_UnsavedDocument); // Submit all _expected_ closure from last frame
...
static void ImGui::DockNodeStartMouseMovingWindow(ImGuiDockNode *node, ImGuiWindow *window) {
  ...
  g.MovingWindow =
      window; // If we are docked into a non moveable root window, StartMouseMovingWindow() won't
              // set g.MovingWindow. Override that decision.
...
static void ImGui::DockNodeUpdateForRootNode(ImGuiDockNode *node) {
  ...
  // - Find if there's only a single visible window in the hierarchy (in which case we need to
  // display a regular title bar -> FIXME-DOCK: that last part is not done yet!)
  ...
  node->OnlyNodeWithWindows =
      (results.CountNodesWithWindows == 1) ? results.FirstNodeWithWindows : NULL;
  ...
  // When node has mixed windows, prioritize the class with the most constraint
  // (DockingAllowUnclassed = false) as the reference to copy.
...
static void ImGui::DockNodeUpdate(ImGuiDockNode *node) {
  ...
  // Early out for hidden root dock nodes (when all DockId references are in inactive windows, or
  // there is only 1 floating window holding on the DockId)
  ...
  // In some circumstance we will defer creating the host window (so everything will be kept
  // hidden), while the expected visible window is resizing itself. This is important for first-time
  // (no ini settings restored) single window when io.ConfigDockingAlwaysTabBar is enabled,
  // otherwise the node ends up using the minimum window size. Effectively those windows will take
  // an extra frame to show up:
  //   N+0: Begin(): window created (with no known size), node is created
  //   N+1: DockNodeUpdate(): node skip creating host window / Begin(): window size applied, not
  //   visible N+2: DockNodeUpdate(): node can create host window / Begin(): window becomes visible
  // We could remove this frame if we could reliably calculate the expected window size during node
  // update, before the Begin() code. It would require a generalization of CalcWindowExpectedSize(),
  // probably extracting code away from Begin(). In reality it isn't very important as user quickly
  // ends up with size data in .ini file.
  if (node->IsVisible && node->HostWindow == NULL && node->IsFloatingNode() && node->IsLeafNode()) {
    IM_ASSERT(node->Windows.Size > 0);
    ImGuiWindow *ref_window = NULL;
    if (node->SelectedTabId != 0) // Note that we prune single-window-node settings on .ini loading,
                                  // so this is generally 0 for them!
  ...
  if (node->IsDockSpace()) {
    ...
    node->EnableCloseButton = false;
    node->HasCloseButton =
        (node->Windows.Size > 0) && (node_flags & ImGuiDockNodeFlags_NoCloseButton) == 0;
    node->HasWindowMenuButton =
        (node->Windows.Size > 0) && (node_flags & ImGuiDockNodeFlags_NoWindowMenuButton) == 0;
    for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
      // FIXME-DOCK: Setting DockIsActive here means that for single active window in a leaf node,
    ...
    if (node->IsRootNode() && node->IsVisible) {
      ...
      // We set ImGuiWindowFlags_NoFocusOnAppearing because we don't want the host window to take
      // full focus (e.g. steal NavWindow) But we still it bring it to the front of display. There's
      // no way to choose this precise behavior via window flags. One simple case to ponder if:
      // window A has a toggle to create windows B/C/D. Dock B/C/D together, clear the toggle and
      // enable it again. When reappearing B/C/D will request focus and be moved to the top of the
      // display pile, but they are not linked to the dock host window during the frame they appear.
      // The dock host window would keep its old display order, and the sorting in EndFrame would
  ...
  // ImGuiDockNodeFlags_PassthruCentralNode, but we will only know the correct pos/size after
  // processing the resizing splitters. So we are using the DrawList channel splitting facility to
  // submit drawing primitives out of order!
  ...
  if (central_node_hole_register_hit_test_hole) {
    // Add a little padding to match the "resize from edges" behavior and allow grabbing the
    // splitter easily.
    IM_ASSERT(
        node->IsDockSpace()); // We cannot pass this flag without the DockSpace() api. Testing this
                              // because we also setup the hole in host_window->ParentNode
    ImRect central_hole(central_node->Pos, central_node->Pos + central_node->Size);
    central_hole.Expand(ImVec2(-WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS,
                               -WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS));
    if (central_node_hole && !central_hole.IsInverted()) {
      ...
      SetWindowHitTestHole(host_window->ParentWindow, central_hole.Min,
                           central_hole.Max - central_hole.Min);
    }
  }
...
// Compare TabItem nodes given the last known DockOrder (will persist in .ini file as hint), used to
// sort tabs when multiple tabs are added on the same frame.
...
static ImGuiID ImGui::DockNodeUpdateWindowMenu(ImGuiDockNode *node, ImGuiTabBar *tab_bar) {
  ...
  if (g.Style.WindowMenuButtonPosition == ImGuiDir_Left)
    SetNextWindowPos(ImVec2(node->Pos.x, node->Pos.y + GetFrameHeight()), ImGuiCond_Always,
                     ImVec2(0.0f, 0.0f));
  else
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode *node, ImGuiWindow *host_window) {
  ...
  // This is to facilitate computing those ID from the outside, and will affect more or less only
  // the ID of the collapse button, popup and tabs, as docked windows themselves will override the
  // stack with their own root ID.
  ...
  // Submit new tabs and apply NavWindow focus back to the tab bar. They will be added as Unsorted
  // and sorted below based on relative DockOrder value.
  ...
  // If multiple tabs are appearing on the same frame, sort them based on their persistent DockOrder
  // value
  int tabs_unsorted_start = tab_bar->Tabs.Size;
  for (int tab_n = tab_bar->Tabs.Size - 1;
       tab_n >= 0 && (tab_bar->Tabs[tab_n].Flags & ImGuiTabItemFlags_Unsorted); tab_n--) {
    // FIXME-DOCK: Consider only clearing the flag after the tab has been alive for a few
    // consecutive frames, allowing late comers to not break sorting?
  ...
  for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
    ...
    if (window->LastFrameActive + 1 >= g.FrameCount || !node_was_active) {
      ...
      bool tab_open = true;
      TabItemEx(tab_bar, window->Name, window->HasCloseButton ? &tab_open : NULL, tab_item_flags,
                window);
      if (!tab_open) node->WantCloseTabId = window->ID;
      if (tab_bar->VisibleTabId == window->ID) node->VisibleWindow = window;

      // Store last item data so it can be queried with IsItemXXX functions after the user Begin()
      // call
  ...
  // Note that VisibleWindow may have been overrided by CTRL+Tabbing, so VisibleWindow->ID may be !=
  // from tab_bar->SelectedTabId
  if (has_close_button && node->VisibleWindow) {
    ...
    if (CloseButton(host_window->GetID("#CLOSE"),
                    title_bar_rect.GetTR() +
                        ImVec2(-style.FramePadding.x * 2.0f - button_sz, 0.0f)))
  ...
  // FIXME: TabItem use AllowItemOverlap so we manually perform a more specific test for now
  // (hovered || held)
...
static void ImGui::DockNodeCalcTabBarLayout(const ImGuiDockNode *node, ImRect *out_title_rect,
  ...
  if (node->HasCloseButton) {
    r.Max.x -= g.FontSize; // +1.0f; // In DockNodeUpdateTabBar() we currently display a disabled
                           // close button even if there is none.
  }
  if (node->HasWindowMenuButton && g.Style.WindowMenuButtonPosition == ImGuiDir_Left) {
    r.Min.x += g.FontSize; // + g.Style.ItemInnerSpacing.x; // <-- Adding ItemInnerSpacing makes the
                           // title text moves slightly when in a docking tab bar. Instead we
                           // adjusted RenderArrowDockMenu()
...
void ImGui::DockNodeCalcSplitRects(ImVec2 &pos_old, ImVec2 &size_old, ImVec2 &pos_new,
                                   ImVec2 &size_new, ImGuiDir dir, ImVec2 size_new_desired) {
  ImGuiContext &  g            = *GImGui;
...
bool ImGui::DockNodeCalcDropRectsAndTestMousePos(const ImRect &parent, ImGuiDir dir, ImRect &out_r,
  ...
  if (outer_docking) {
    // hs_w = ImFloor(ImClamp(parent_smaller_axis - hs_for_central_nodes * 4.0f, g.FontSize * 0.5f,
    // g.FontSize * 8.0f)); hs_h = ImFloor(hs_w * 0.15f); off = ImVec2(ImFloor(parent.GetWidth() *
    // 0.5f - GetFrameHeightWithSpacing() * 1.4f - hs_h), ImFloor(parent.GetHeight() * 0.5f -
    // GetFrameHeightWithSpacing() * 1.4f - hs_h));
    hs_w = ImFloor(hs_for_central_nodes * 1.50f);
    hs_h = ImFloor(hs_for_central_nodes * 0.80f);
    off =
        ImVec2(ImFloor(parent.GetWidth() * 0.5f - hs_h), ImFloor(parent.GetHeight() * 0.5f - hs_h));
  } else {
...
static void ImGui::DockNodePreviewDockSetup(ImGuiWindow *host_window, ImGuiDockNode *host_node,
  ...
  // Because the inactive leaf node doesn't have proper pos/size yet, we'll use the root node as
  // reference.
  ...
  ImGuiDockNodeFlags src_node_flags = root_payload_as_host
                                          ? root_payload_as_host->GetMergedFlags()
                                          : root_payload->WindowClass.DockNodeFlagsOverrideSet;
  ImGuiDockNodeFlags dst_node_flags =
      host_node ? host_node->GetMergedFlags() : host_window->WindowClass.DockNodeFlagsOverrideSet;
  ...
  // Build a tentative future node (reuse same structure because it is practical. Shape will be
  // readjusted when previewing a split)
  ...
  if (!host_window->Collapsed)
    for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++) {
      ...
      if (DockNodeCalcDropRectsAndTestMousePos(data->FutureNode.Rect(), (ImGuiDir)dir,
                                               data->DropRectsDraw[dir + 1], is_outer_docking,
                                               &g.IO.MousePos)) {
  ...
  // When docking without holding Shift, we only allow and preview docking when hovering over a drop
  // rect or over the title bar
...
static void ImGui::DockNodePreviewDockRender(ImGuiWindow *host_window, ImGuiDockNode *host_node,
  ...
  // With this option, we only display the preview on the target viewport, and the payload viewport
  // is made transparent. To compensate for the single layer obstructed by the payload, we'll
  // increase the alpha of the preview nodes.
  const bool is_transparent_payload = g.IO.ConfigDockingTransparentPayload;

  // In case the two windows involved are on different viewports, we will draw the overlay on each
  // of them.
  ...
  const ImU32 overlay_col_lines =
      GetColorU32(ImGuiCol_NavWindowingHighlight, is_transparent_payload ? 0.80f : 0.60f);
  ...
  if (data->IsDropAllowed) {
    ...
    if (data->SplitDir != ImGuiDir_None || data->IsCenterAvailable)
      for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
        overlay_draw_lists[overlay_n]->AddRectFilled(overlay_rect.Min, overlay_rect.Max,
                                                     overlay_col_main, host_window->WindowRounding);
  }

  // Display tab shape/label preview unless we are splitting node (it generally makes the situation
  // harder to read)
  if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None &&
    ...
    if (host_node && host_node->TabBar) {
      if (!host_node->IsHiddenTabBar() && !host_node->IsNoTabBar())
        tab_pos.x += host_node->TabBar->OffsetMax +
                     g.Style.ItemInnerSpacing
                         .x; // We don't use OffsetNewTab because when using non-persistent-order
      ...
      tab_pos.x += g.Style.ItemInnerSpacing.x +
                   TabItemCalcSize(host_window->Name, host_window->HasCloseButton)
                       .x; // Account for slight offset which will be added when changing from title
                           // bar to tab bar
    }

    // Draw tab shape/label preview (payload may be a loose window or a host window carrying
    ...
    const int payload_count =
        root_payload->DockNodeAsHost ? root_payload->DockNodeAsHost->TabBar->Tabs.Size : 1;
    for (int payload_n = 0; payload_n < payload_count; payload_n++) {
      ...
      for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++) {
        ImGuiTabItemFlags tab_flags =
            ...
            ((payload->Flags & ImGuiWindowFlags_UnsavedDocument) ? ImGuiTabItemFlags_UnsavedDocument
                                                                 : 0);
        if (!tab_bar_rect.Contains(tab_bb))
          overlay_draw_lists[overlay_n]->PushClipRect(tab_bar_rect.Min, tab_bar_rect.Max);
        TabItemBackground(overlay_draw_lists[overlay_n], tab_bb, tab_flags, overlay_col_tabs);
        TabItemLabelAndCloseButton(overlay_draw_lists[overlay_n], tab_bb, tab_flags,
                                   g.Style.FramePadding, payload->Name, 0, 0, false);
  ...
  for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++) {
    if (!data->DropRectsDraw[dir + 1].IsInverted()) {
      ...
      for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++) {
        ...
        overlay_draw_lists[overlay_n]->AddRectFilled(draw_r.Min, draw_r.Max, overlay_col,
                                                     overlay_rounding);
    ...
    if ((host_node && (host_node->GetMergedFlags() & ImGuiDockNodeFlags_NoSplit)) ||
...
void ImGui::DockNodeTreeSplit(ImGuiContext *ctx, ImGuiDockNode *parent_node, ImGuiAxis split_axis,
  ...
  ImGuiDockNode *child_0 =
      (new_node && split_inheritor_child_idx != 0) ? new_node : DockContextAddNode(ctx, 0);
  child_0->ParentNode = parent_node;

  ImGuiDockNode *child_1 =
      (new_node && split_inheritor_child_idx != 1) ? new_node : DockContextAddNode(ctx, 0);
  ...
  IM_ASSERT(size_avail > 0.0f); // If you created a node manually with DockBuilderAddNode(), you
...
void ImGui::DockNodeTreeUpdatePosSize(ImGuiDockNode *node, ImVec2 pos, ImVec2 size,
  ...
  // 'only_write_to_marked_nodes' is only set when turning a node visible mid-frame and we need its
  // size right-away.
  ...
  if (child_0->IsVisible && child_1->IsVisible) {
    ...
    // 2) Process locked absolute size (during a splitter resize we preserve the child of nodes not
    // touching the splitter edge)
    ...
    // 3) If one window is the central node (~ use remaining space, should be made explicit!), use
    // explicit size from the other, and remainder for the central node
      ...
      float split_ratio =
          child_0->SizeRef[axis] / (child_0->SizeRef[axis] + child_1->SizeRef[axis]);
...
static void DockNodeTreeUpdateSplitterFindTouchingNode(ImGuiDockNode *node, ImGuiAxis axis,
                                                       int                        side,
...
void ImGui::DockNodeTreeUpdateSplitter(ImGuiDockNode *node) {
  ...
  ImGuiDockNode *child_1 = node->ChildNodes[1];
  if (child_0->IsVisible && child_1->IsVisible) {
    // Bounding box of the splitter cover the space between both nodes (w = Spacing, h = Size[xy^1]
    // for when splitting horizontally)
    const ImGuiAxis axis = (ImGuiAxis)node->SplitAxis;
    ...
    if ((merged_flags & ImGuiDockNodeFlags_NoResize) || (merged_flags & no_resize_axis_flag)) {
      ...
      // bb.Min[axis] += 1; // Display a little inward so highlight doesn't connect with nearby tabs
      // on the neighbor node. bb.Max[axis] -= 1;
      PushID(node->ID);

      // Gather list of nodes that are touching the splitter line. Find resizing limits based on
      // those nodes.
      ...
      resize_limits[1] =
          node->ChildNodes[1]->Pos[axis] + node->ChildNodes[1]->Size[axis] - min_size;
      ...
      if (g.ActiveId == splitter_id) {
        ...
        GetForegroundDrawList((ImGuiViewportP*)GetMainViewport()); for (int n = 0; n < 2; n++) if
        (axis == ImGuiAxis_X) draw_list->AddLine(ImVec2(resize_limits[n],
        node->ChildNodes[n]->Pos.y), ImVec2(resize_limits[n], node->ChildNodes[n]->Pos.y +
        node->ChildNodes[n]->Size.y), IM_COL32(255, 0, 255, 255), 3.0f); else
                draw_list->AddLine(ImVec2(node->ChildNodes[n]->Pos.x, resize_limits[n]),
        ImVec2(node->ChildNodes[n]->Pos.x + node->ChildNodes[n]->Size.x, resize_limits[n]),
        IM_COL32(255, 0, 255, 255), 3.0f);
        */
      }

      // Use a short delay before highlighting the splitter (and changing the mouse cursor) in order
      // for regular mouse movement to not highlight many splitters
      float cur_size_0 = child_0->Size[axis];
      float cur_size_1 = child_1->Size[axis];
      float min_size_0 = resize_limits[0] - child_0->Pos[axis];
      float min_size_1 = child_1->Pos[axis] + child_1->Size[axis] - resize_limits[1];
      if (SplitterBehavior(bb, GetID("##Splitter"), axis, &cur_size_0, &cur_size_1, min_size_0,
                           min_size_1, WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS,
        ...
        if (touching_nodes[0].Size > 0 && touching_nodes[1].Size > 0) {
          ...
          // This might be less desirable if we can merge sibling of a same axis into the same
          // parental level.
...
void ImGui::SetWindowDock(ImGuiWindow *window, ImGuiID dock_id, ImGuiCond cond) {
  ...
  // If the user attempt to set a dock id that is a split node, we'll dig within to find a suitable
  // docking spot
...
// Create an explicit dockspace node within an existing window. Also expose dock node flags and
// creates a CentralNode by default. The Central Node is always displayed even when empty and
// shrink/extend according to the requested size of its neighbors. DockSpace() needs to be submitted
// _before_ any window they can host. If you use a dockspace, submit it early in your app.
void ImGui::DockSpace(ImGuiID id, const ImVec2 &size_arg, ImGuiDockNodeFlags flags,
  ...
  // This is faster but also DockNodeUpdateTabBar() relies on TabBarLayout() running (which won't if
  // SkipItems=true) to set NextSelectedTabId = 0). See #2960. If for whichever reason this is
  // causing problem we would need to ensure that DockNodeUpdateTabBar() ends up clearing
  // NextSelectedTabId even if SkipItems=true.
  ...
  // It is possible that the node has already been claimed by a docked window which appeared before
  // the DockSpace() node, so we overwrite IsDockSpace again.
  ...
  // Keep alive mode, this is allow windows docked into this node so stay docked even if they are
  // not visible
  ...
  // FIXME-DOCK Why do we need a child window to host a dockspace, could we host it in the existing
  // window?
  ...
  // This can happen if the node was first created manually with DockBuilderAddNode() but _without_
  // the ImGuiDockNodeFlags_Dockspace. Doing it correctly would set the _CentralNode flags, which
  // would then propagate according to subsequent split. It would also be ambiguous to attempt to
  // assign a central node while there are split nodes, so we wait until there's a single node
  // remaining. The specific sub-property of _CentralNode we are interested in recovering here is
  // the "Don't delete when empty" property, as it doesn't make sense for an empty dockspace to not
  // have this property.
...
// Even though we could pass window flags, it would also require the user to be able to call
// BeginMenuBar() somehow meaning we can't Begin/End in a single function. But you can also use
// BeginMainMenuBar(). If you really want a menu bar inside the same window as the one hosting the
// dockspace, you will need to copy this code somewhere and tweak it.
...
// Make sure to use the ImGuiDockNodeFlags_DockSpace flag to create a dockspace node! Otherwise this
// will create a floating node!
// - Floating node: you can then call DockBuilderSetNodePos()/DockBuilderSetNodeSize() to position
// and size the floating node.
// - Dockspace node: calling DockBuilderSetNodePos() is unnecessary.
// - If you intend to split a node immediately after creation using DockBuilderSplitNode(), make
// sure to call DockBuilderSetNodeSize() beforehand!
//   For various reason, the splitting code currently needs a base size otherwise space may not be
//   allocated as precisely as you would expect.
...
ImGuiID ImGui::DockBuilderAddNode(ImGuiID id, ImGuiDockNodeFlags flags) {
  ...
  node->LastFrameAlive =
      ctx->FrameCount; // Set this otherwise BeginDocked will undock during the same frame.
  return node->ID;
}
...
void ImGui::DockBuilderRemoveNodeChildNodes(ImGuiID root_id) {
  ...
  for (int n = 0; n < dc->Nodes.Data.Size; n++)
    if (ImGuiDockNode *node = (ImGuiDockNode *)dc->Nodes.Data[n].val_p) {
      bool want_removal =
          (root_id == 0) || (node->ID != root_id && DockNodeGetRootNode(node)->ID == root_id);
      if (want_removal) {
  ...
  // DockNodeMoveWindows->DockNodeAddWindow will normally set those when reaching two windows (which
  // is only adequate during interactive merge) Make sure we don't lose our current pos/size.
  // (FIXME-DOCK: Consider tidying up that code in DockNodeAddWindow instead)
  if (root_node) {
  ...
  // Not really efficient, but easier to destroy a whole hierarchy considering DockContextRemoveNode
  // is attempting to merge nodes
...
// If 'out_id_at_dir' or 'out_id_at_opposite_dir' are non NULL, the function will write out the ID
// of the two new nodes created. Return value is ID of the node at the specified direction, so same
// as (*out_id_at_dir) if that pointer is set.
...
ImGuiID ImGui::DockBuilderSplitNode(ImGuiID id, ImGuiDir split_dir,
  ...
  req.DockSplitDir     = split_dir;
  req.DockSplitRatio   = ImSaturate((split_dir == ImGuiDir_Left || split_dir == ImGuiDir_Up)
  ...
  ImGuiID id_at_dir =
      node->ChildNodes[(split_dir == ImGuiDir_Left || split_dir == ImGuiDir_Up) ? 0 : 1]->ID;
...
// FIXME: Will probably want to change this signature, in particular how the window remapping pairs
// are passed.
void ImGui::DockBuilderCopyDockSpace(ImGuiID src_dockspace_id, ImGuiID dst_dockspace_id,
  ...
  // FIXME: When overwriting dst_dockspace_id, windows that aren't part of our dockspace window
  // class but that are docked in a same node will be split apart, whereas we could attempt to at
  // least keep them together in a new, same floating node.
  ImVector<ImGuiID> node_remap_pairs;
  DockBuilderCopyNode(src_dockspace_id, dst_dockspace_id, &node_remap_pairs);

  // Attempt to transition all the upcoming windows associated to dst_dockspace_id into the newly
  // created hierarchy of dock nodes (The windows associated to src_dockspace_id are staying in
  // place)
  ...
  for (int remap_window_n = 0; remap_window_n < in_window_remap_pairs->Size; remap_window_n += 2) {
    ...
    if (dst_dock_id != 0) {
      ...
      IMGUI_DEBUG_LOG_DOCKING("Remap live window '%s' 0x%08X -> '%s' 0x%08X\n", src_window_name,
                              src_dock_id, dst_window_name, dst_dock_id);
      DockBuilderDockWindow(dst_window_name, dst_dock_id);
    } else {
      // Floating windows gets their settings transferred (regardless of whether the new window
      // already exist or not) When this is leading to a Copy and not a Move, we would get two
      // overlapping floating windows. Could we possibly dock them together?
  ...
  // Anything else in the source nodes of 'node_remap_pairs' are windows that were docked in
  // src_dockspace_id but are not owned by it (unaffiliated windows, e.g. "ImGui Demo") Find those
  // windows and move to them to the cloned dock node. This may be optional?
  for (int dock_remap_n = 0; dock_remap_n < node_remap_pairs.Size; dock_remap_n += 2)
...
bool ImGui::GetWindowAlwaysWantOwnTabBar(ImGuiWindow *window) {
  ...
  if (g.IO.ConfigDockingAlwaysTabBar || window->WindowClass.DockingAlwaysTabBar)
    if ((window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_NoTitleBar |
      ...
      if (!window->IsFallbackWindow) // We don't support AlwaysTabBar on the fallback/implicit
                                     // window to avoid unused dock-node overhead/noise
        return true;
  return false;
}
...
static ImGuiDockNode *ImGui::DockContextBindNodeToWindow(ImGuiContext *ctx, ImGuiWindow *window) {
  ...
  // DockNodeTreeUpdatePosSize() yet, so we're forcing a Pos/Size update from the first ancestor
  // that is already visible (often it will be the root node). If we don't do this, the window will
  // be assigned a zero-size on its first frame, which won't ideally warm up the layout. This is a
  // little wonky because we don't normally update the Pos/Size of visible node mid-frame.
  if (!node->IsVisible) {
...
#  endif
  ...
  // Note how we are testing for LastFrameAlive and NOT LastFrameActive. A DockSpace node can be
  // maintained alive while being inactive with ImGuiDockNodeFlags_KeepAliveOnly.
  ...
  // Fast path return. It is common for windows to hold on a persistent DockId but be the only
  // visible window, and never create neither a host window neither a tab bar.
  // FIXME-DOCK: replace ->HostWindow NULL compare with something more explicit (~was initially
  // intended as a first frame test)
  ...
  window->Flags |= ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_AlwaysUseWindowPadding |
                   ImGuiWindowFlags_NoResize;
  if (node->IsHiddenTabBar() || node->IsNoTabBar())
    window->Flags |= ImGuiWindowFlags_NoTitleBar;
  else
    window->Flags &=
        ~ImGuiWindowFlags_NoTitleBar; // Clear the NoTitleBar flag in case the user set it:
  ...
  // This allows multiple windows to be created in the same frame and have their respective dock
  // orders preserved.
...
void ImGui::BeginDockableDragDropSource(ImGuiWindow *window) {
  ...
  if (is_drag_docking && BeginDragDropSource(ImGuiDragDropFlags_SourceNoPreviewTooltip |
...
void ImGui::BeginDockableDragDropTarget(ImGuiWindow *window) {
  ...
  // Peek into the payload before calling AcceptDragDropPayload() so we can handle overlapping dock
  // nodes with filtering (this is a little unusual pattern, normally most code would call
  // AcceptDragDropPayload directly)
  ...
  if (AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_WINDOW,
    ...
    // Select target node
    // (we should not assume that g.HoveredDockNode is != NULL when window is a host dock node: it
    // depends on padding/spacing handled by DockNodeTreeFindVisibleNodeByPos)
    ImGuiDockNode *node               = g.HoveredDockNode;
    const bool allow_null_target_node = window->DockNode == NULL && window->DockNodeAsHost == NULL;

    // There is an edge case when docking into a dockspace which only has inactive nodes (because
    // none of the windows are active) In this case we need to fallback into any leaf mode, possibly
    // the central node.
    if (window->DockNodeAsHost && node && node->IsDockSpace() && node->IsRootNode()) {
      if (node->CentralNode &&
          node->IsLeafNode()) // FIXME-20181220: We should not have to test for IsLeafNode() here
    ...
    // const bool do_preview = true;     // Ignore testing for payload->IsPreview() which removes
    // one frame of delay, but breaks overlapping drop targets within the same window.
    ...
    if (do_preview && (node != NULL || allow_null_target_node)) {
      ...
      if (node && (node->ParentNode || node->IsCentralNode()))
        if (ImGuiDockNode *root_node = DockNodeGetRootNode(node)) {
          DockNodePreviewDockSetup(window, root_node, payload_window, &split_outer,
                                   is_explicit_target, true);
          if (split_outer.IsSplitDirExplicit) split_data = &split_outer;
        }
      DockNodePreviewDockSetup(window, node, payload_window, &split_inner, is_explicit_target,
                               false);
      if (split_data == &split_outer) split_inner.IsDropAllowed = false;

      // Draw inner then outer, so that previewed tab (in inner data) will be behind the outer drop
      // boxes
...
static void *ImGui::DockSettingsHandler_ReadOpen(ImGuiContext *, ImGuiSettingsHandler *,
                                                 const char *name) {
  if (strcmp(name, "Data") != 0) return NULL;
  return (void *)1;
}

static void ImGui::DockSettingsHandler_ReadLine(ImGuiContext *ctx, ImGuiSettingsHandler *, void *,
                                                const char *  line) {
  char c = 0;
  int  x = 0, y = 0;
  int  r = 0;
...
static void DockSettingsHandler_DockNodeToSettings(ImGuiDockContext *dc, ImGuiDockNode *node,
                                                   int depth) {
...
static void ImGui::DockSettingsHandler_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler,
  ...
  // (unlike our windows settings, because nodes are always built we can do a full rewrite of the
  // SettingsNode buffer)
  ...
  for (int node_n = 0; node_n < dc->NodesSettings.Size; node_n++) {
    ...
    if (node_settings->ParentNodeId) {
      ...
      buf->appendf(" Pos=%d,%d Size=%d,%d", node_settings->Pos.x, node_settings->Pos.y,
...
static void RenderViewportThumbnail(ImDrawList *draw_list, ImGuiViewportP *viewport,
                                    const ImRect &bb) {
  ImGuiContext &g      = *GImGui;
  ImGuiWindow * window = g.CurrentWindow;
  ...
  window->DrawList->AddRectFilled(bb.Min, bb.Max,
                                  ImGui::GetColorU32(ImGuiCol_Border, alpha_mul * 0.40f));
  for (int i = 0; i != g.Windows.Size; i++) {
    ImGuiWindow *thumb_window = g.Windows[i];
    if (!thumb_window->WasActive || ((thumb_window->Flags & ImGuiWindowFlags_ChildWindow)))
      continue;
    if (thumb_window->SkipItems &&
        (thumb_window->Flags & ImGuiWindowFlags_ChildWindow)) // FIXME-DOCK: Skip hidden docked
                                                              // windows. Identify those betters.
      continue;
    ...
    ImRect thumb_r_scaled =
        ImRect(ImFloor(off + thumb_r.Min * scale), ImFloor(off + thumb_r.Max * scale));
    ...
    window->DrawList->AddRectFilled(
        ...
        ImGui::GetColorU32(window_is_focused ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg,
                           alpha_mul));
    ...
    if (ImGuiWindow *window_for_title = GetWindowForTitleDisplay(thumb_window))
      window->DrawList->AddText(g.Font, g.FontSize * 1.0f, title_r_scaled.Min,
                                ImGui::GetColorU32(ImGuiCol_Text, alpha_mul),
                                window_for_title->Name,
...
void ImGui::ShowViewportThumbnails() {
  ...
  // We don't display full monitor bounds (we could, but it often looks awkward), instead we display
  // just enough to cover all of our viewports.
...
void ImGui::ShowMetricsWindow(bool *p_open) {
  ...
  struct Funcs {
    ...
    static void NodeDrawCmdShowMeshAndBoundingBox(ImDrawList *      fg_draw_list,
                                                  const ImDrawList *draw_list,
                                                  const ImDrawCmd *draw_cmd, int elem_offset,
      ...
      for (unsigned int base_idx = elem_offset; base_idx < (elem_offset + draw_cmd->ElemCount);
           base_idx += 3) {
        ImVec2 triangle[3];
        for (int n = 0; n < 3; n++) {
          ImVec2 p =
              draw_list->VtxBuffer[idx_buffer ? idx_buffer[base_idx + n] : (base_idx + n)].pos;
          triangle[n] = p;
          vtxs_rect.Add(p);
        }
        if (show_mesh)
    ...
    static void NodeDrawList(ImGuiWindow *window, ImGuiViewportP *viewport, ImDrawList *draw_list,
      ...
      for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end();
        ...
        if (ImGui::IsItemHovered() && (show_drawcmd_mesh || show_drawcmd_aabb) && fg_draw_list)
          NodeDrawCmdShowMeshAndBoundingBox(fg_draw_list, draw_list, pcmd, elem_offset,
        ...
        for (unsigned int base_idx = elem_offset; base_idx < (elem_offset + pcmd->ElemCount);
             base_idx += 3) {
          ImVec2 triangle[3];
          for (int n = 0; n < 3; n++)
            triangle[n] =
        ...
        if (ImGui::IsItemHovered() && fg_draw_list)
          NodeDrawCmdShowMeshAndBoundingBox(fg_draw_list, draw_list, pcmd, elem_offset, true,
                                            false);
    ...
    static void NodeWindow(ImGuiWindow *window, const char *label) {
      ...
      ImGui::BulletText("Active: %d/%d, WriteAccessed: %d, BeginOrderWithinContext: %d",
      ...
      ImGui::BulletText("ViewportMonitor: %d",
                        window->Viewport ? window->Viewport->PlatformMonitor : -1);
      ImGui::BulletText("DockId: 0x%04X, DockOrder: %d, Act: %d, Vis: %d", window->DockId,
    ...
    static void NodeViewport(ImGuiViewportP *viewport) {
      ...
      if (ImGui::TreeNode((void *)(intptr_t)viewport->ID,
        ...
        ImGui::BulletText(
            "Flags: 0x%04X =%s%s%s%s%s%s%s", viewport->Flags,
        ...
        for (int layer_i = 0; layer_i < IM_ARRAYSIZE(viewport->DrawDataBuilder.Layers); layer_i++)
          for (int draw_list_i = 0; draw_list_i < viewport->DrawDataBuilder.Layers[layer_i].Size;
               draw_list_i++)
            Funcs::NodeDrawList(NULL, viewport,
    ...
    static void NodeDockNode(ImGuiDockNode *node, const char *label) {
      ...
      if (node->Windows.Size > 0)
        open =
            ImGui::TreeNode((void *)(intptr_t)node->ID, "%s 0x%04X%s: %d windows (vis: '%s')",
                            label, node->ID, node->IsVisible ? "" : " (hidden)", node->Windows.Size,
                            node->VisibleWindow ? node->VisibleWindow->Name : "NULL");
      else
        open = ImGui::TreeNode((void *)(intptr_t)node->ID, "%s 0x%04X%s: %s split (vis: '%s')",
      ...
      if (open) {
        ...
        ImGui::BulletText("Pos (%.0f,%.0f), Size (%.0f, %.0f) Ref (%.0f, %.0f)", node->Pos.x,
                          node->Pos.y, node->Size.x, node->Size.y, node->SizeRef.x,
                          node->SizeRef.y);
        ...
        ImGui::BulletText("Misc:%s%s%s%s%s", node->IsDockSpace() ? " IsDockSpace" : "",
  ...
  if (ImGui::TreeNode("Tools")) {
    ...
    if (show_windows_rects && g.NavWindow) {
      ...
      for (int rect_n = 0; rect_n < WRT_Count; rect_n++) {
        ...
        ImGui::Text("(%6.1f,%6.1f) (%6.1f,%6.1f) Size (%6.1f,%6.1f) %s", r.Min.x, r.Min.y, r.Max.x,
  ...
  if (ImGui::TreeNode("Viewport", "Viewports (%d)", g.Viewports.Size)) {
    ...
    ImGui::SameLine();
    MetricsHelpMarker("Dear ImGui uses monitor data:\n- to query DPI settings on a per monitor "
    ...
    if (open) {
      for (int i = 0; i < g.PlatformIO.Monitors.Size; i++) {
        ...
        ImGui::BulletText(
            ...
            mon.MainPos.y + mon.MainSize.y, mon.MainSize.x, mon.MainSize.y, mon.WorkPos.x,
            mon.WorkPos.y, mon.WorkPos.x + mon.WorkSize.x, mon.WorkPos.y + mon.WorkSize.y,
...
#    ifdef IMGUI_HAS_DOCK
  if (ImGui::TreeNode("Dock nodes")) {
    ...
    for (int n = 0; n < dc->Nodes.Data.Size; n++)
      if (ImGuiDockNode *node = (ImGuiDockNode *)dc->Nodes.Data[n].val_p)
        if (!root_nodes_only || node->IsRootNode()) Funcs::NodeDockNode(node, "Node");
    ImGui::TreePop();
  }
#    endif // #ifdef IMGUI_HAS_DOCK
        ...
        if (settings->SelectedWindowId) {
          if (ImGuiWindow *window = FindWindowByID(settings->SelectedWindowId))
            selected_tab_name = window->Name;
                       ...
                       FindWindowSettings(settings->SelectedWindowId))
...
#    endif // #ifdef IMGUI_HAS_DOCK
  ...
  if (ImGui::TreeNode("Internal state")) {
    ...
    ImGui::Text(
        ...
        g.HoveredIdAllowOverlap); // Data is "in-flight" so depending on when the Metrics window is
    ...
    ImGui::Text("MouseViewport: 0x%08X (UserHovered 0x%08X, LastHovered 0x%08X)",
